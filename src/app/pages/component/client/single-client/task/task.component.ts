...
export class AllTaskComponent implements OnInit, AfterViewInit {
  ...
  taskdata(data){
    // console.log(data);
    this.task = data;
    this.router.navigate(['/single-client-task']);
  }

  backToClient(){
    this.client = this.order.project.client;
  }
  backToProject(){
 ...
 findLocation(){
     // console.log(this.project.longitude);
     this.comService.redrawMap(this.map, this.project.lattitude, this.project.longitude);
  }
