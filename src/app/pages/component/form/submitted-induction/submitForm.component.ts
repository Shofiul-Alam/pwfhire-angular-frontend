...
export class SubmittedListComponent implements OnInit, AfterViewInit {
  formRender: any; loader:boolean = false;
  modal=null; modalElpe = null; 
  private updateFieldValue;
  formDataList:Array<UserSubmittedInduction>; private formList:Array<Form> = [];
  form:Form; private editForm:UserSubmittedInduction;
  pdfData; formName;
  ...
  ngOnInit(){
    ...
    this.getAllSubmittedInductions();
  ...
  getAllSubmittedInductions() {
    this._inductionService.getAllSubmittedInductions().subscribe(
        response => {
  ...
  edit(f){
   ...
   this._formService.getSingleForm(formId).subscribe(
    ...
    // console.log(form); 
    let renderedForm = this._formService.initForSubmitEdit(form,this.editForm.induction.form);
  ...
  updateData(){
    ...
    this.editForm.induction.form.fieldsArr = this._formService.getUpdatedSubmittedInduction(this.formList,fields);
